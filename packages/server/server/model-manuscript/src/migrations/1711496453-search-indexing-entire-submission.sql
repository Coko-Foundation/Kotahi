-- Function to recursively process and concatenate text values from JSONB, handling strings, objects, and arrays
CREATE OR REPLACE FUNCTION concatenate_text_values(jsonb_value JSONB) RETURNS TEXT AS $$
DECLARE
  result_text TEXT := '';
  element JSONB;
BEGIN
  ...
  ELSIF jsonb_typeof(jsonb_value) = 'array' THEN
    FOR element IN SELECT value FROM jsonb_array_elements(jsonb_value) LOOP
      result_text := result_text || concatenate_text_values(element);
    END LOOP;
  END IF;
  RETURN result_text;
...
BEGIN
  ...
  -- plus the manuscript's short ID, and all text fields (and sub-fields) from the submission object not covered above.
  b := concat_ws(E'\n',
    authorstext,
    memberstext,
    inviteestext,
