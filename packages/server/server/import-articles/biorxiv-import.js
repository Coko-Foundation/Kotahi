...
const getData = async (groupId, ctx, options = {}) => {
  return useTransaction(
    async trx => {
      ...
      const [biorxivImportSourceId] = await models.ArticleImportSources.query(
        trx,
      ).where({
        server: 'biorxiv',
      })
      ...
      const date = lastImportDate.length
        ? lastImportDate[0].date
        : dateTwoWeeksAgo
      ...
      try {
        const inserted = await models.Manuscript.query(trx).upsertGraphAndFetch(
          newManuscripts,
          { relate: true },
        )
