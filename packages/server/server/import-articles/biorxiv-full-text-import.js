...
const getData = async (groupId, ctx, options = {}) => {
  return useTransaction(
    async trx => {
      const sourceId = await getServerId('biorxiv', { trx })
      const lastImportDate = await getLastImportDate(sourceId, groupId, { trx })
      ...
      const importsWithDesiredCategoryOnly = withoutDuplicates.filter(
        preprint => allowedSubjectMatterAreas.includes(preprint.category),
      )
      ...
      try {
        ...
        for (let i = 0; i < newManuscripts.length; i += SAVE_CHUNK_SIZE) {
          ...
          const inserted = await models.Manuscript.query(
            trx,
          ).upsertGraphAndFetch(chunk, { relate: true })
