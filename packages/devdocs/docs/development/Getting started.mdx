...
* A recent version of `docker` (eg. 29 as of this writing)
* `node` >= 24
* `yarn` version 4.15.0
* `git`
...
The only way to currently log in to Kotahi is by using ORCID. This makes it mandatory
...
(we'll use `myelifetestemail` for this guide) and click "GO".
* You'll be taken to a new page. This is your inbox for myelifetestemail@mailinator.com. 
:::tip[Tip]

When entering your username, choose a username that is unlikely to be used already by someone else
...
* Under "Redirect URIs", add the url of your Kotahi server plus `/auth/orcid/callback`. 
For development, you'll probably be running the server on localhost, but ORCID 
requires a valid url, so replace that with 127.0.0.1. So if in your browser you 
can get your server status under http://localhost:3000/healthcheck, the value here should be 
http://127.0.0.1:3000/auth/orcid/callback.
* Click on the floating save icon on the right.

:::info Note
ORCID supports multiple 
