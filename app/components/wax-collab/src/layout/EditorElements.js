...
// Theoretically they could do it with CSS referencing Google fonts--is that the way we recommend?
// That might break PDF export in PagedJS if that doesn't get fixed.
...
const EditorStyles = async () => {
  const textStyles = await getTextStyles() // this needs an await to work! But if we include an async, the styled component doesn't work
  return css`
