...
const sequenceMapping = {
  ...
  const requestToCrossref = async (xmlFiles) => {
    const publishPromises = xmlFiles.map(async file => {
      ...
      const crossrefURL = process.env.NODE_ENV === 'production' ? 'https://doi.crossref.org/servlet/deposit' : 'https://test.crossref.org/servlet/deposit'
  ...
  const publishToCrossref = async manuscript => {
    ...
    const xmls = notEmptyReviews.map(reviewNumber => {
      if(!manuscript.submission['review' + reviewNumber + 'date']) {
        return null
      }
      const [ month, day, year ] = manuscript.submission['review' + reviewNumber + 'date'].split('/')
      ...
      if (manuscript.submission[`review${reviewNumber}creator`]) {
        const surname = manuscript.submission[`review${reviewNumber}creator`].split(
          ' ',
        )[1]
      ...
      `${process.env.PUBLIC_CLIENT_PROTOCOL}://${process.env.PUBLIC_CLIENT_HOST}:${process.env.PUBLIC_CLIENT_PORT}/kotahi/versions/${manuscript.id}/article-evaluation-result/${reviewNumber}`
    ...
    if (manuscript.submission.summary && manuscript.submission.summarydate) {
      ...
      if (manuscript.submission.summarycreator) {
        ...
        templateCopy.doi_batch.body[0].peer_review[0].contributors[0].person_name[0] = {
          ...
          given_name: [
            manuscript.submission.summarycreator.split(' ')[0],
          ],
          surname: [surname ? surname : ''],
        }
      }
      ...
      templateCopy.doi_batch.body[0].peer_review[0].doi_data[0].resource[0] =
      `${process.env.PUBLIC_CLIENT_PROTOCOL}://${process.env.PUBLIC_CLIENT_HOST}:${process.env.PUBLIC_CLIENT_PORT}/kotahi/versions/${manuscript.id}/article-evaluation-summary`
      templateCopy.doi_batch.body[0].peer_review[0].program[0].related_item[0] = {
        // description: [`${manuscript.submission.description}`],
        inter_work_relation: [
          {
    ...
    const fileCreationPromises = xmls.map(async xml => {
        const fileName = xml.reviewNumber ? 'review' + xml.reviewNumber + '.xml' : 'summary.xml'
