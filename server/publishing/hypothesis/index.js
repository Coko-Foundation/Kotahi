...
const deletePublication = (publicationId) => {
  return axios.delete(`https://api.hypothes.is/api/annotations/${publicationId}`, {...headers, data: {}})
}
...
const publishToHypothesis = async (manuscript) => {
    ...
    turndownService.addRule('unorderedLists', {
      ...
      if(!manuscript.evaluationsHypothesisMap.summary && !checkIsAbstractValueEmpty(manuscript.submission.summary)) {
        //Create summary
      ...
      const createPromises = shouldCreateReviews.map(propName => {
        const requestBody = {
          ...
          text: turndownService.turndown(
            manuscript.submission[propName],
          ),
      ...
      const updatePromises = shouldUpdateReviews.map(propName => {
        ...
        const publicationId = manuscript.evaluationsHypothesisMap[propName]
        return axios.patch(requestURL + `/${publicationId}`, requestBody, headers).then(() => ({
      ...
      const newHypothesisEvaluationMap = [...createResults, ...deleteResults, ...updateResults].reduce((acc,curr) => {
        return {
          ...acc,
          ...curr
        }
