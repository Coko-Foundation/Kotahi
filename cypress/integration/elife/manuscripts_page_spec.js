...
describe('Manuscripts page tests', () => {
  ...
  context('unsubmitetd article tests', () => {
    ...
    it('sort article after Article id', () => {
      ...
      Menu.clickManuscripts()
      ManuscriptsPage.getArticleIdByRow(0).should('contain', 'def')
      ManuscriptsPage.getArticleIdByRow(1).should('contain', 'abc')
      ManuscriptsPage.getArticleIdByRow(2).should('contain', '456')
      ManuscriptsPage.getArticleIdByRow(3).should('contain', '123')
      ManuscriptsPage.clickTableHead(0)
      ManuscriptsPage.getArticleIdByRow(0).should('contain', '123')
      ManuscriptsPage.getArticleIdByRow(1).should('contain', '456')
      ManuscriptsPage.getArticleIdByRow(2).should('contain', 'abc')
      ManuscriptsPage.getArticleIdByRow(3).should('contain', 'def')
    })
  })
  ...
  context('Submited and evaluated article tests', () => {
    ...
    it('assert atricle id is the first table head and contains submitted atricle id title', () => {
      ...
      // eslint-disable-next-line jest/valid-expect-in-promise
      cy.fixture('submission_form_data').then(data => {
        ManuscriptsPage.getArticleIdByRow(0).should('contain', data.articleId)
      })
    })
  })
})
